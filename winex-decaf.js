...
const WORKER_ID = (function() {
  let left;
  const wid = (left = process.env.NODE_WORKER_ID != null ? process.env.NODE_WORKER_ID : (cluster.worker != null ? cluster.worker.id : undefined)) != null ? left : null;
  if (wid) { return `w${wid}`; } else { return "m"; }
})();
...
let _Log      = {error(...args) { return console.log.apply(this, ["[ERROR]"].concat(args)); }};
...
const uncaughtLogger = function(err, cleanup) {
  let log;
  if (cleanup == null) { cleanup = function() {}; }
  try {
    log = new _Log({
    ...
    return console.log((other ? (other.stack != null ? other.stack : other) : "Unknown"));
  }
  finally {
...
const stripNulls = function(obj) {
  for (let k of Object.keys(obj || {})) { const v = obj[k]; if (v === null) { delete obj[k]; } }
  return obj;
};
...
const clientIp = function(req) {
  let ipAddr    = __guard__(req != null ? req.connection : undefined, x => x.remoteAddress);
  const forwards  = __guardMethod__(req, 'header', o => o.header("x-forwarded-for"));

  if (forwards) {
    // Use header forwards if possible.
    const ips     = forwards.split(",");
    const firstIp = ((ips != null ? ips[0] : undefined) != null ? (ips != null ? ips[0] : undefined) : "").replace(/^\s+|\s+$/, "");
    if (firstIp) { ipAddr  = firstIp; }
  }
  ...
  class Log {
    static initClass() {
      ...
      this.patchMeta = (...metas) => {
        ...
        metas = ((() => {
          const result = [];
          for (let m of Array.from(metas)) {             if (m != null) {
              result.push(m);
            }
          }
          return result;
    ...
    constructor(opts) {
      ...
      if (opts.req) { this.addReq(opts.req); }
      if (opts.res) { this.addRes(opts.res); }
      if (opts.error) { this.addError(opts.error); }
    }
    ...
    addReq(req) {
      ...
      let query       = (urlObj != null ? urlObj.query : undefined) != null ? (urlObj != null ? urlObj.query : undefined) : "";
      const queryChars  = query != null ? query.length : undefined;
      const pathLength  = (path != null ? path.length : undefined) != null ? (path != null ? path.length : undefined) : 0;
      if (pathLength > maxChars) { path        = path.substr(0, maxChars); }
      if (queryChars > maxChars) { query       = query.substr(0, maxChars); }

      return this.meta = extend(this.meta, stripNulls({
        reqClient:      clientIp(req),
        reqHost:        __guard__(req != null ? req.headers : undefined, x => x.host) != null ? __guard__(req != null ? req.headers : undefined, x => x.host) : null,
        reqMethod:      (req != null ? req.method : undefined) != null ? (req != null ? req.method : undefined) : null,
        reqPath:        path,
        reqQuery:       query,
        reqQueryChars:  queryChars,
        reqUser:        __guard__(req != null ? req.user : undefined, x1 => x1.email) != null ? __guard__(req != null ? req.user : undefined, x1 => x1.email) : null
      })
      );
    }
    ...
    addError(error) {
      ...
      const errObj = {errMsg: (error != null ? error.message : undefined) != null ? (error != null ? error.message : undefined) : error.toString()};

      if (!this.errNoStack) {
        extend(errObj, stripNulls({
          errArgs:  ((error != null ? error.arguments : undefined) != null ? (error != null ? error.arguments : undefined) : "").toString().substr(0, 100),
          errType:  (error != null ? error.type : undefined) != null ? (error != null ? error.type : undefined) : null,
          errStack: (error != null ? error.stack : undefined) != null ? (error != null ? error.stack : undefined) : null,
          errKnown: 0
        })
        );
      }
    ...
    baseMeta(meta, level = null) {
      ...
      return extend({
        ...
        type:       (meta != null ? meta.type : undefined) != null ? (meta != null ? meta.type : undefined) : this.DEFAULT_TYPE,
    ...
    _makeMeta(level, meta) {
      ...
      let left, left1;
      if (meta == null) { meta = {}; }
      const type = (left = (left1 = meta.type != null ? meta.type : this.meta.type) != null ? left1 : this.type) != null ? left : this.DEFAULT_TYPE;
