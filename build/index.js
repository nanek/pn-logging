...
const winex_1 = __importDefault(require("./winex"));
const logform_1 = require("logform"); //depdency of wintson, ensure we are using the same version as winston
const { combine, json, prettyPrint } = logform_1.format;
var sysLogLevels;
...
function Log(options) {
    ...
    if (options.rawTransports) {
        ...
        logTransports = new winston_1.default.transports.Console({
        ...
        logTransports = options.transports.map(function (t) {
            ...
            var Transport = winston_1.default.transports[cls];
            if (Transport == winston_1.default.transports.Console) {
    ...
    winstonLog = winston_1.default.createLogger(winstonOpts);
    this._winexConstructor = winex_1.default.factory(winstonLog, options.meta);
    /*
...
function _logger(level) {
    return function (message, meta, error, req, res) {
        ...
        if (util_1.default.types.isNativeError(meta)) {
            error = meta;
            meta = null;
        }
        if (meta) {
